 ...
 * Not cached -- fetches credentials on every call so rotated keys are picked up.
 *
 * We rely on Replit's "managed webhook" flow (findOrCreateManagedWebhook), so
 * no webhook signing secret needs to be supplied here -- StripeSync looks up
 * the secret for managed webhooks internally.
 */
