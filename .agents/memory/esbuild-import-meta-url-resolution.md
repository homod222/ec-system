...
**How to apply:** when integrating a package that touches the filesystem for its own assets (migrations, templates, worker threads — pino transports have the same issue), add it to the bundler's `external` list rather than letting it get inlined. This is a generic bundler-interaction lesson, not specific to any one package — watch for it whenever a "it runs but does nothing" symptom shows up after adding bundling/esbuild to a project that previously ran unbundled.
